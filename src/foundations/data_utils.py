...
class DatasetManager:
    ...
    def get_sample_text_data(self, num_samples: int = 1000) -> dict[str, Any]:
        ...
        templates = {"positive": positive_templates, "negative": negative_templates, "neutral": neutral_templates}
        ...
        return {"texts": texts, "labels": labels, "label_names": categories, "num_classes": len(categories)}

    def get_sample_tabular_data(
        self, num_samples: int = 1000, num_features: int = 10, task_type: str = "classification"
    ) -> dict[str, Any]:
        """
    ...
    def get_sample_timeseries_data(
        self, num_series: int = 100, series_length: int = 100, num_features: int = 1
    ) -> dict[str, Any]:
        """
    ...
    def create_train_val_test_split(
        ...
        for split_name, idx in [("train", train_idx), ("val", val_idx), ("test", test_idx)]:
            ...
            splits[split_name] = {"X": split_X, "y": split_y, "size": len(idx)}
...
def create_pytorch_dataloader(
    data_split: dict[str, Any], batch_size: int = 32, shuffle: bool = True, num_workers: int = 0
) -> DataLoader:
...
def create_tensorflow_dataset(
    data_split: dict[str, Any], batch_size: int = 32, shuffle: bool = True, buffer_size: int = 1000
):
