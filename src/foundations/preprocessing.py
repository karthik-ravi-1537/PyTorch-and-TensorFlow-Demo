...
class TextPreprocessor:
    ...
    def build_vocabulary(self, texts: list[str], min_freq: int = 1, max_vocab_size: int | None = None):
    ...
    def texts_to_sequences(self, texts: list[str], max_length: int | None = None) -> list[list[int]]:
...
class TabularPreprocessor:
    ...
    def scale_numerical_features(
        self, X: np.ndarray, feature_names: list[str], method: str = "standard", fit: bool = True
    ) -> np.ndarray:
    ...
    def encode_categorical_features(
        self, X: np.ndarray, feature_names: list[str], method: str = "onehot", fit: bool = True
    ...
    def preprocess_tabular_data(
        ...
        # Encode categorical features
        X_encoded, new_feature_names = self.encode_categorical_features(X_array, feature_names, encoding_method, fit)

        # Scale numerical features
        X_processed = self.scale_numerical_features(X_encoded, new_feature_names, scaling_method, fit)
...
class TimeSeriesPreprocessor:
    ...
    def normalize_series(
        self, series: np.ndarray, method: str = "standard", fit: bool = True, feature_axis: int = -1
    ) -> np.ndarray:
    ...
    def create_sequences(
        self, data: np.ndarray, sequence_length: int, prediction_horizon: int = 1, step_size: int = 1
    ...
    def add_time_features(self, timestamps: np.ndarray) -> np.ndarray:
        ...
        time_features = np.column_stack(
            [timestamps.hour, timestamps.day, timestamps.month, timestamps.dayofweek, timestamps.dayofyear]
        )

        return time_features

    def handle_missing_values(self, series: np.ndarray, method: str = "interpolate") -> np.ndarray:
...
def pad_sequences_pytorch(sequences: list[list[int]], max_length: int | None = None, padding_value: int = 0):
...
def pad_sequences_tensorflow(sequences: list[list[int]], max_length: int | None = None, padding: str = "post"):
    """Pad sequences for TensorFlow."""
    if not TENSORFLOW_AVAILABLE:
        raise ImportError("TensorFlow not available")

    return tf.keras.preprocessing.sequence.pad_sequences(sequences, maxlen=max_length, padding=padding)
...
def preprocess_tabular_data(X, feature_names=None, scaling="standard", encoding="onehot"):
    ...
    X_processed, new_feature_names = preprocessor.preprocess_tabular_data(X, feature_names, scaling, encoding)

    return {"X_processed": X_processed, "feature_names": new_feature_names, "preprocessor": preprocessor}
...
def preprocess_timeseries_data(series, sequence_length=50, normalize=True):
    ...
    return {"X": X, "y": y, "preprocessor": preprocessor}
