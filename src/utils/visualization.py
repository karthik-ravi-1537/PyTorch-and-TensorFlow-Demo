...
class FrameworkVisualizer:
    ...
    def plot_performance_comparison(
        self, benchmark_results: dict[str, Any], title: str = "Performance Comparison", save_path: str | None = None
    ) -> plt.Figure:
        """
        ...
        for framework in ["pytorch", "tensorflow"]:
            if framework in benchmark_results and benchmark_results[framework].get("success"):
        ...
        if not frameworks:
            ax1.text(0.5, 0.5, "No successful benchmarks", ha="center", va="center", transform=ax1.transAxes)
            ax2.text(0.5, 0.5, "No data available", ha="center", va="center", transform=ax2.transAxes)
            return fig

        # Bar plot of mean execution times
        colors = [self.colors[fw.lower()] for fw in frameworks]
        bars = ax1.bar(frameworks, mean_times, yerr=std_times, capsize=5, color=colors, alpha=0.7)
        ax1.set_ylabel("Execution Time (seconds)")
        ax1.set_title("Mean Execution Time")
        ax1.grid(True, alpha=0.3)

        # Add value labels on bars
        for bar, mean_time, std_time in zip(bars, mean_times, std_times, strict=False):
            height = bar.get_height()
            ax1.text(
                bar.get_x() + bar.get_width() / 2.0, height + std_time, f"{mean_time:.4f}s", ha="center", va="bottom"
            )
        ...
        for framework in ["pytorch", "tensorflow"]:
            if (
                framework in benchmark_results
                and benchmark_results[framework].get("success")
                and "times" in benchmark_results[framework]
            ):
    ...
    def plot_training_curves(
        self,
        pytorch_history: dict[str, list[float]] | None = None,
        tensorflow_history: dict[str, list[float]] | None = None,
        ...
        for i, metric in enumerate(metrics):
            ...
            if pytorch_history and metric in pytorch_history:
                ...
                ax.plot(
                    epochs,
                    pytorch_history[metric],
                    color=self.colors["pytorch"],
                    linewidth=2,
                    label=f"PyTorch {metric}",
                    marker="o",
                    markersize=4,
                )
            ...
            if tensorflow_history and metric in tensorflow_history:
                ...
                ax.plot(
                    epochs,
                    tensorflow_history[metric],
                    color=self.colors["tensorflow"],
                    linewidth=2,
                    label=f"TensorFlow {metric}",
                    marker="s",
                    markersize=4,
                )
    ...
    def plot_memory_usage(
        self,
        pytorch_memory: list[float] | None = None,
        tensorflow_memory: list[float] | None = None,
    ...
    def plot_accuracy_comparison(
        ...
        if not comparison:
            ax1.text(0.5, 0.5, "No comparison data available", ha="center", va="center", transform=ax1.transAxes)
        ...
        # Add text labels
        for i, (_metric, value) in enumerate(zip(metrics, values, strict=False)):
            ax1.text(i, 0.5, str(value), ha="center", va="center", fontweight="bold", color="white")
        ...
        if max_diff is not None and mean_diff is not None:
            ...
            ax2.text(0.5, 0.5, "No numerical differences available", ha="center", va="center", transform=ax2.transAxes)
    ...
    def _plot_execution_status(self, ax, results):
        ...
        for bar, status in zip(bars, statuses, strict=False):
            ax.text(
    ...
    def _plot_accuracy_metrics(self, ax, results):
        ...
        for i, (_metric, value) in enumerate(zip(metrics, values, strict=False)):
            ax.text(i, 0.5, str(value), ha="center", va="center", fontweight="bold", color="white")
    ...
    def _plot_framework_info(self, ax, results):
        ...
        ax.text(0.1, 0.9, "\n".join(info_text), transform=ax.transAxes, verticalalignment="top", fontfamily="monospace")
    ...
    def _plot_performance_summary(self, ax, benchmark_results):
        ...
        for fw in ["pytorch", "tensorflow"]:
            if fw in benchmark_results and benchmark_results[fw].get("success"):
        ...
        if frameworks:
            ...
            for bar, time in zip(bars, mean_times, strict=False):
                ax.text(bar.get_x() + bar.get_width() / 2.0, bar.get_height(), f"{time:.4f}s", ha="center", va="bottom")
        else:
